...
def get_results_imputation(missing_data_percentage, imputation_method):
    ...
    for i in range(num_iterations):
        ...
        for j, filename in enumerate(files):
            ...
            missing_values = [{'position': i, 'value': dataset_orig[i]} for i, value in enumerate(dataset_missing_data) if np.isnan(value)]

            results['number_missing_values'] += len(missing_values)

            for missing_value in missing_values:
                imputed_value = new_dataset[missing_value['position']]
                squared_error = pow((imputed_value - missing_value['value']), 2)

                results['sum_squared_error'] += squared_error

        t1 = time.time()

        print("Duração: {}s".format(t1 - t0))
...
imputation_strategies = [InterpolationImputation]
...
for missing_data_percentage in list_missing_data_percentage:
    ...
    for imputation_strategy in imputation_strategies:
        ...
        results_estimation = get_results_imputation(missing_data_percentage, imputation_strategy)
